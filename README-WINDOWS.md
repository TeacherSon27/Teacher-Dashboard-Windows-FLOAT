...
In Points mode, the highest possible point value is adjustable from 1 to 1000. The setting applies to both Add and Deduct rolls and is saved with the other WHO'S LUCKY controls. Changing the highest point value never resets or changes the accumulated point total. Values above 100 still reuse the fixed 100-image ball set for animation and display while the generated numeric point value can reach the configured maximum.
...
WHO'S LUCKY Points mode is bounded in the renderer to prevent invalid point ranges: the highest value clamps to 1 through 1000, uses direct random integer generation instead of loading extra assets, and keeps the existing fixed visible-ball animation pool. The accumulated total is stored separately from the range setting, so changing the range does not mutate scoring history or trigger extra roll work.
